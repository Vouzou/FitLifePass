...
function pmpro_getAvailableCreditsForUser( $user_id = null, $include_inactive = false ) {
	...
	$available_credits = $wpdb->get_results("SELECT mu.available_credits AS credits FROM {$wpdb->pmpro_memberships_users} AS mu WHERE mu.user_id = $user_id");
	console.log('db credits: ' + $available_credits->credits);
