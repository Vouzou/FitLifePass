...
<table class="form-table">
    <?php
		...
		<tr>
			<th><label for="available_credits"><?php _e("Available Credits", 'paid-memberships-pro' ); ?></label></th>
			<td>
				<input id="available_credits" name="available_credits" type="text" size="10" value="
				<?php
					echo $user->membership_level->available_credits
				?>" />
			</td>
		</tr>
    ...
    <script>
        jQuery(document).ready(function() {
			...
			var $available_credits = jQuery("[name=available_credits]")
			...
			var old_available_credits = $available_credits.val();
			console.log('old_available_credits: ' + old_available_credits);
			...
			function pmpro_checkForLevelChangeInProfile()
			{
				...
				//did level or credits or expiration change?
                if(
					...
					$available_credits.val() != old_available_credits
				)
                {
			...
			$available_credits.change(function() {
				pmpro_checkForLevelChangeInProfile();
			});
...
function pmpro_membership_level_profile_fields_update()
{
	...
	//credits change
	if(isset($_REQUEST['available_credits']))
	{
		//update available credits
		$available_credits = intval($_REQUEST['available_credits']);
		$sqlQuery = "UPDATE $wpdb->pmpro_memberships_users SET available_credits = '" . $available_credits . "' WHERE status = 'active' AND membership_id = '" . intval($_REQUEST['membership_level']) . "' AND user_id = '" . $user_ID . "' LIMIT 1";
		if($wpdb->query($sqlQuery))
			$available_credits_changed = true;
	}
	...
	if(!empty($level_changed) || !empty($expiration_changed) || !empty($available_credits_changed))
	{
		//email to admin
